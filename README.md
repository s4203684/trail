...
Logging a session is just picking the topic you worked on and noting what you did — no timer, no duration to track.
...
### Content-based tracking (no hours, no timer)

StudyTrail tracks **what** you studied, not how long. Logging a session means picking the topic you worked on (chip or dropdown) and writing what you did/next/notes — no timer runs, no duration is recorded. "How much" progress you've made is read off each topic's **status** (`todo` → `doing` → `done`, or `stuck`), which you set from the session modal or the topic row directly.

The Report page (`report.js`) reflects this: no total/average time. It shows sessions logged, topics/subjects touched, a sessions-per-day activity chart, a **Needs Focus** list (topics not yet done, least-progressed first — based on status), **Coverage by subject** (topics done/total per subject), top topics by session count, and personal bests (most-studied subject, topic closest to finishing, total topics completed).
