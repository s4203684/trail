...
const App = (() => {
  ...
  function openCheckout() {
    ...
    const topic = subj ? subj.topics.find(t => t.id === active.topicId) : null;
    ...
    document.getElementById('co-session-info').innerHTML = `
      <div class="card-paper" style="margin-bottom:20px">
        ...
        <div class="font-medium mt-4">${subj ? UI.esc(subj.name) : ''} → ${topic ? UI.esc(topic.name) : ''}</div>
    ...
    if (statusEl && topic) statusEl.value = topic.status;
  ...
  function bindCheckoutForm() {
    document.getElementById('co-submit').addEventListener('click', () => {
      ...
      // Update topic status
      if (newStatus) ST.updateTopic(active.subjectId, active.topicId, { status: newStatus });

      // If there's a stumble note, save it
      if (stumbled) {
        ST.addStumble(active.subjectId, active.topicId, 'other', stumbled);
      }
