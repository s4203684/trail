...
function renderBackupReminder() {
  ...
  const backupRaw = localStorage.getItem('st2_backup');
  let backupAge = null;
  if (backupRaw) {
    try {
      const parsed = JSON.parse(backupRaw);
      if (parsed.backedUpAt) backupAge = Math.floor((Date.now() - new Date(parsed.backedUpAt).getTime()) / 86400000);
    } catch(e) {}
  }
  const noExport = !lastExport;
  const exportOld = lastExport ? ((Date.now() - new Date(lastExport).getTime()) / 86400000) >= 5 : false;
  if (!noExport && !exportOld && !(backupAge !== null && backupAge >= 3)) {
    el.innerHTML = '';
    return;
  }
  el.innerHTML = `<div class="smart-banner">
    <div class="smart-banner-title">💾 Download a backup</div>
    <div class="smart-banner-text">You already know sync can fail sometimes. Keep a fresh backup file on your laptop or phone.</div>
    <div class="insight-meta"><button class="btn btn-green btn-sm" onclick="exportData()">Download backup</button><button class="btn btn-ghost btn-sm" onclick="showPage('settings')">Open settings</button></div>
  </div>`;
}
...
function showPage(name) {
  ...
  if (name === 'settings')  { document.querySelector('[onclick="showPage(\'settings\')"]').classList.add('active'); document.getElementById('settings-name').value = getName(); updateBackupStatus(); renderBackupReminder(); }
...
function renderSubjects() {
  ...
  if (orderedSubjects.length === 0) {
    list.innerHTML = `<div class="empty"><div class="empty-icon">📚</div><div class="empty-text">Start by adding your first subject.<br>Then log one session so StudyTrail can guide what to focus on next.</div></div>`;
    return;
  }
  ...
  list.innerHTML = orderedSubjects.map(s => {
    ...
    const smartPills = [];
    if (days !== null && days >= 3) smartPills.push(`<span class="smart-pill smart-danger">⚠ Losing progress</span>`);
    if (s.nextTodo) smartPills.push(`<span class="smart-pill smart-good">👉 Continue from here</span>`);
    if (deadlineDays !== null && deadlineDays <= 3) smartPills.push(`<span class="smart-pill smart-warn">🚨 Focus now</span>`);
